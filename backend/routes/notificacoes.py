...
from notificacoes import enviar_alerta_frequencia, enviar_alerta_risco, notifier
...
def listar_alertas(
    ...
    query = query.filter(models.Predicao.nivel_risco.in_(["ALTO", "CRITICO"]))
        ...
        alertas.append(
            {
                ...
                "data_predicao": (
                    predicao.data_predicao.isoformat() if predicao.data_predicao else None
                ),
            }
        )
...
def enviar_alerta_frequencia_endpoint(
    ...
    if sucesso:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="❌ Erro ao enviar alerta"
        )
...
def enviar_alerta_geral(
    ...
    sucesso = enviar_alerta_risco(dados_aluno, float(predicao.risco_evasao), predicao.nivel_risco)

...
def enviar_alerta_faltas_consecutivas(
    ...
    alerta = (
        ...
        .options(joinedload(models.AlertaFaltasConsecutivas.aluno).joinedload(models.Aluno.curso))
    ...
    _mensagem = notifier.gerar_mensagem_faltas_seguidas(dados_aluno, int(alerta.quantidade_faltas))
    ...
    try:
        comunicacao = disparar_mensagem_unificado(
            ...
            contexto={
                ...
                "data_inicio": (
                    alerta.data_inicio_faltas.strftime("%d/%m/%Y")
                    if alerta.data_inicio_faltas
                    else "N/A"
                ),
                "data_fim": (
                    alerta.data_fim_faltas.strftime("%d/%m/%Y") if alerta.data_fim_faltas else "N/A"
                ),
            },
            canal="TELEGRAM",

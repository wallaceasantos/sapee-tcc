...
# ============================================
# CONFIGURAÇÕES DE PESOS (Carregáveis via ml_config.json)
# ============================================
...
_PESOS_DEFAULT = {
    ...
    # ============================================
    "score_base": 5,
    # Fatores Acadêmicos (peso dominante — maior poder discriminativo)
    "frequencia_baixa_60": 30,
    "frequencia_baixa_75": 22,
    "frequencia_baixa_85": 10,
    "media_critica_4": 28,
    "media_baixa_5": 20,
    "media_baixa_6": 12,
    "reprovas_muitas_3": 16,
    "reprovas_algumas_1": 12,
    # Fatores Socioeconômicos
    "renda_muito_baixa_1000": 8,
    "renda_baixa_1500": 5,
    "trabalho_tempo_integral_40": 12,
    "trabalho_meio_periodo_20": 8,
    "trabalho_parcial": 2,
    # Fatores de Deslocamento (baixo poder discriminativo — peso reduzido)
    "deslocamento_critico_120": 0,
    "deslocamento_elevado_60": 0,
    "acesso_muito_dificil": 10,
    "acesso_dificil": 2,
    # Fatores de Infraestrutura (sem poder discriminativo — peso reduzido)
    "sem_computador": 0,
    "sem_internet": 0,
    # Fatores de Vulnerabilidade (sem poder discriminativo — peso reduzido)
    "bolsa_familia": 0,
    "primeira_geracao": 0,
    # ============================================
    # FATORES DE PROTEÇÃO (reduzem o risco) - reforçados na recalibração
    # ============================================
    "frequencia_excelente_90": -18,
    "frequencia_boa_85": -6,
    "media_excelente_8": -14,
    "media_boa_7": -6,
    "sem_reprovas": -6,
    "renda_alta_3000": -2,
    "nao_trabalha": -4,
    ...
    # ============================================
    "psicossocial_intencao_evasao_alta": 18,
    "psicossocial_intencao_evasao_media": 10,
    "psicossocial_saude_mental_critica": 15,
    "psicossocial_saude_mental_ruim": 10,
    "psicossocial_integracao_baixa": 8,
    "psicossocial_conflitos_altos": 8,
    "psicossocial_satisfacao_baixa": 8,
    # Fatores de Tendência (Bônus/Malus)
    "queda_frequencia_brusca": 18,
    "queda_frequencia_suave": 10,
    "melhora_frequencia": -8,
    "queda_media_brusca": 18,
    "queda_media_suave": 10,
    "melhora_media": -8,
}

# Combinações perigosas (multiplicadores) - Ajustados para maior sensibilidade
_COMBINACOES_DEFAULT = {
    "frequencia_baixa_e_trabalha": 1.2,
    "media_baixa_e_reprovas": 1.2,
    "trabalha_e_deslocamento_longo": 1.1,
    "vulnerabilidade_multipla": 1.1,
    "queda_dupla": 1.25,
}
...
def _carregar_config() -> dict:
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            logger.warning("Erro ao carregar ml_config.json: %s. Usando defaults.", e)
    return {}


_config = _carregar_config()
PESOS_FATORES = _config.get("pesos_fatores", _PESOS_DEFAULT)
COMBINACOES_PERIGOSAS = _config.get("combinacoes_perigosas", _COMBINACOES_DEFAULT)
LIMIARES_RISCO = _config.get("limiares_risco", _LIMIARES_DEFAULT)
SCORE_COMPRESSION = _config.get("score_compression", _COMPRESSION_DEFAULT)
SCORE_NORMALIZACAO = _config.get("score_normalizacao", {})
_RAW_CAP = SCORE_NORMALIZACAO.get("raw_cap")
...
def carregar_modelo():
    ...
    if os.path.exists(MODEL_PATH):
        try:
            ...
            logger.warning("Erro ao carregar modelo ML: %s", e)
    return None
...
def predicao_com_modelo(aluno_data, model):
        ...
        [
            {
                ...
                "renda_familiar": (
                    float(aluno_data.renda_familiar) if aluno_data.renda_familiar else 3000
                ),
    ...
    nivel = (
        ...
        if risco_final > 70
        else models.NivelRisco.MEDIO if risco_final > 30 else models.NivelRisco.BAIXO
    )
...
def fallback_logic_v2(aluno_data, db=None):
            ...
            fatores_detalhados.append(
                {
                    ...
                    "descricao": "Tempo de deslocamento crítico (>2h/dia)",
                    "severidade": "ALERTA",
                }
            )
            ...
            fatores_detalhados.append(
                {
                    ...
                    "descricao": "Tempo de deslocamento elevado (>1h/dia)",
                    "severidade": "ATENÇÃO",
                }
            )
    ...
    if db and aluno_data.matricula:
        ...
        historico = (
            ...
            .filter(models.FrequenciaMensal.aluno_id == aluno_data.matricula)
    ...
    if db and aluno_data.matricula:
        try:
            questionario = (
                ...
                .filter(models.QuestionarioPsicossocial.aluno_matricula == aluno_data.matricula)
    ...
    if (
        aluno_data.media_geral
        and float(aluno_data.media_geral) < 5
        and aluno_data.historico_reprovas
        and aluno_data.historico_reprovas > 1
    ):
    ...
    # Calibração do score: normaliza o score bruto (soma ponderada) para a escala
    # 0-100 sem saturação. O `raw_cap` representa o score bruto que corresponde a
    # 100 pontos. Quando não configurado, mantém a compressão legada.
    if _RAW_CAP:
        score = 100.0 * score / float(_RAW_CAP)
        score = max(0, min(score, 100))
    else:
        # Aplicar escala suave para scores muito altos (evita saturação em 100)
        if score > SCORE_COMPRESSION["threshold_1"]:
            score = SCORE_COMPRESSION["threshold_1"] + (score - SCORE_COMPRESSION["threshold_1"]) * SCORE_COMPRESSION["factor_1"]
        if score > SCORE_COMPRESSION["threshold_2"]:
            score = SCORE_COMPRESSION["threshold_2"] + (score - SCORE_COMPRESSION["threshold_2"]) * SCORE_COMPRESSION["factor_2"]
        score = max(0, min(score, 100))
    ...
    return {
        ...
        "nivel_risco": nivel,
        "fatores_principais": (
            ", ".join(fatores_principais)
            if fatores_principais
            else "Sem fatores de risco identificados"
        ),
...
def gerar_explicacao(resultado):
    ...
    if nivel == models.NivelRisco.BAIXO:
        return (
            f"Aluno apresenta baixo risco de evasão ({risco:.1f}%). Fatores protetivos predominam."
        )
...
def gerar_explicacao_v2(score, nivel, fatores_detalhados):
    ...
    fatores_risco = [
        f for f in fatores_detalhados if f.get("severidade") in ["CRÍTICA", "ALERTA", "ATENÇÃO"]
    ]
    ...
    if not fatores_risco and fatores_protecao:
        ...
        explicacao += "✅ Aluno apresenta perfil acadêmico favorável.\n"
        explicacao += "Fatores protetores identificados:\n"

...
def testar_telegram():
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/v1/notificacoes/testar-telegram", headers=headers, timeout=10
        )
...
def testar_alerta_geral(matricula: str):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/v1/notificacoes/alerta-geral/{matricula}", headers=headers, timeout=10
        )

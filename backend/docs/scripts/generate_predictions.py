...
def gerar_predicoes():
    ...
    try:
        ...
        # ============================================
        alunos_com_predicao = db.query(models.Predicao.aluno_id).distinct().subquery()

        alunos_sem_predicao = (
            db.query(models.Aluno).filter(~models.Aluno.matricula.in_(alunos_com_predicao)).all()
        )
        ...
        # Alunos sem predição
        alunos_com_predicao = db.query(models.Predicao.aluno_id).distinct().count()
        ...
        if risco_alto + risco_medio + risco_baixo != alunos_com_predicao:
            print(
                f"⚠️  ATENÇÃO: Soma dos riscos ({risco_alto + risco_medio + risco_baixo}) ≠ Com predição ({alunos_com_predicao})!"
            )
        else:

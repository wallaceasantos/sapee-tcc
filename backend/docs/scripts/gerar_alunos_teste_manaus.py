...
NOMES_FEMININOS = [
    ...
    "Pedra Ferreira Rocha",
    "Josefa Barbosa Nunes",
    "Raimunda Mendes Duarte",
    "Sebastiana Araújo Santos",
    "Ana Costa Oliveira",
    "Lucia Souza Pereira",
    "Teresa Lima Rodrigues",
...
for i in range(20):
    ...
    # Gerar endereço
    logradouro = (
        random.choice(["Rua", "Avenida", "Travessa", "Beco"])
        + " "
        + random.choice(
            [
                "das Flores",
    ...
    try:
        ...
        """,
            (
                ...
                aluno_data["frequencia"],
                aluno_data["historico_reprovas"],
                aluno_data["cidade"],
                aluno_data["cep"],
                aluno_data["logradouro"],
                aluno_data["numero"],
                aluno_data["complemento"],
                aluno_data["bairro"],
                aluno_data["zona_residencial"],
                aluno_data["renda_familiar"],
                aluno_data["renda_per_capita"],
                aluno_data["possui_auxilio"],
                aluno_data["trabalha"],
                aluno_data["carga_horaria_trabalho"],
                aluno_data["tempo_deslocamento"],
                aluno_data["custo_transporte_diario"],
                aluno_data["dificuldade_acesso"],
                aluno_data["transporte_utilizado"],
                aluno_data["possui_computador"],
                aluno_data["possui_internet"],
                aluno_data["beneficiario_bolsa_familia"],
        ...
        print(
            f"✅ {matricula} - {nome[:30]:30s} | {curso_nome[:25]:25s} | Risco: {nivel_risco:5s} ({score:5.1f})"
        )
...
for aluno in alunos_gerados:
    ...
    print(
        f"  Frequência: {aluno['frequencia']}% | Média: {aluno['media_geral']} | Reprovas: {aluno['historico_reprovas']}"
    )

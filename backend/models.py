...
class Aluno(Base):
    ...
    parentesco_responsavel_1 = Column(
        String(50), nullable=True, comment="Parentesco do 1º Responsável"
    )
    ...
    parentesco_responsavel_2 = Column(
        String(50), nullable=True, comment="Parentesco do 2º Responsável"
    )
    ...
    frequencias = relationship(
        "FrequenciaMensal", back_populates="aluno", cascade="all, delete-orphan"
    )
    intervencoes = relationship("Intervencao", back_populates="aluno", cascade="all, delete-orphan")
    notas_disciplina = relationship(
        "NotaDisciplina", back_populates="aluno", cascade="all, delete-orphan"
    )
...
class NotaDisciplina(Base):
    ...
    disciplina_id = Column(
        ...
        nullable=True,
        comment="FK para disciplina padronizada",
    )
    periodo_letivo = Column(
        String(20), nullable=False, index=True, comment="Ex: 2024-1, 2024-2, 2025-1"
    )
...
class PredicaoHistorico(Base):
    ...
    aluno_evasao = Column(
        Boolean, default=None, index=True, comment="True se aluno evadiu (abandono)"
    )
    ...
    predicao_correta = Column(
        Boolean, default=None, index=True, comment="True se predição condizia com resultado"
    )
    tipo_erro = Column(
        String(50),
        default=None,
...
class Intervencao(Base):
    ...
    )  # NULL para rascunhos automáticos
    ...
    prioridade = Column(
        Enum("BAIXA", "MEDIA", "ALTA", "URGENTE", name="prioridade_enum"), default="MEDIA"
    )
...
class AlunoMeta(Base):
    ...
    status = Column(
        Enum("PENDENTE", "EM_ANDAMENTO", "ATINGIDA", "NAO_ATINGIDA"), default="PENDENTE"
    )
...
class RegistroFaltasDiarias(Base):
    ...
    disciplina_id = Column(
        ...
        nullable=True,
        index=True,
        comment="ID da disciplina padronizada",
    )
...
class AlertaFaltasConsecutivas(Base):
    ...
    aluno_matricula = Column(String(20), ForeignKey("alunos.matricula"), nullable=False, index=True)
    tipo_alerta = Column(
        Enum("3_FALTAS", "5_FALTAS", "10_FALTAS", name="tipo_alerta_enum"),
        nullable=False,
        index=True,
    )
    ...
    status = Column(
        Enum("PENDENTE", "EM_ANALISE", "RESOLVIDO", "IGNORADO", name="status_alerta_enum"),
        default="PENDENTE",
        index=True,
    )
    ...
    contato_responsavel_data = Column(
        Date, nullable=True, comment="Data do contato com os responsáveis"
    )
    contato_responsavel_meio = Column(
        String(50), nullable=True, comment="Meio de contato: Telefone, WhatsApp, Email, Presencial"
    )
    contato_responsavel_obs = Column(
        Text, nullable=True, comment="Observações sobre o contato com responsáveis"
    )
    ...
    historico = relationship(
        "AlertaFaltasHistorico",
        back_populates="alerta",
        order_by="AlertaFaltasHistorico.criado_at.desc()",
...
class AlertaFaltasHistorico(Base):
    ...
    alerta_id = Column(
        Integer,
        ForeignKey("alertas_faltas_consecutivas.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class QuestionarioPsicossocial(Base):
    ...
    q9_relacionamento_professores = Column(
        Integer, comment="Tenho bom relacionamento com professores"
    )
...
class TokenQuestionario(Base):
    ...
    # Token
    token = Column(
        String(100), unique=True, nullable=False, index=True, comment="Token único de acesso (UUID)"
    )

    # Validação
    valido_ate = Column(
        DateTime(timezone=True), nullable=False, index=True, comment="Data/hora de expiração"
    )
    ...
    # Metadados
    criado_at = Column(
        DateTime(timezone=True), server_default=func.now(), comment="Data de criação"
    )
...
class Egresso(Base):
    ...
    motivo_saida = Column(
        Enum("TRANSFERENCIA", "ABANDONO", "CONCLUSAO", "TRANCAMENTO", "JUBILAMENTO", "OUTROS"),
        nullable=False,
        index=True,
        comment="Motivo principal da saída",
    )
    ...
    motivo_abandono_principal = Column(
        Enum(
            "FINANCEIRO",
            "SAUDE",
            "FAMILIA",
    ...
    situacao_atual = Column(
        Enum("EMPREGADO", "DESEMPREGADO", "ESTUDANDO", "OUTROS"), comment="Situação após conclusão"
    )
    ...
    satisfeito_com_formacao = Column(
        Boolean, default=True, comment="Está satisfeito com a formação"
    )
    recomendaria_curso = Column(Boolean, default=True, comment="Recomendaria o curso")

    # Dados do SAPEE (para medir eficácia)
    tinha_predicao_risco = Column(
        Boolean, default=False, comment="Tinha predição de risco no SAPEE"
    )
    ...
    data_cadastro = Column(
        DateTime(timezone=True), server_default=func.now(), comment="Data de cadastro"
    )
    cadastrado_por = Column(
        Integer, ForeignKey("usuarios.id"), comment="ID do usuário que cadastrou"
    )
    data_atualizacao = Column(
        DateTime(timezone=True), onupdate=func.now(), comment="Data da última atualização"
    )
    atualizado_por = Column(
        Integer, ForeignKey("usuarios.id"), comment="ID do usuário que atualizou"
    )
...
class Atendimento(Base):
    ...
    usuario_id = Column(
        ...
        nullable=False,
        index=True,
        comment="Profissional que registrou",
    )
    ...
    # Encaminhamentos
    necessita_encaminhamento = Column(
        Boolean, default=False, comment="Precisa de encaminhamento externo"
    )
    status_encaminhamento = Column(
        Enum("SOLICITADO", "EM_ATENDIMENTO", "CONCLUIDO", "CANCELADO"),
        nullable=True,
        comment="Status do fluxo de encaminhamento",
    )
    tipo_encaminhamento = Column(
        String(100), nullable=True, comment="Tipo: CAPS, UBS, Conselho Tutelar, etc."
    )
    ...
    # Prioridade
    prioridade = Column(
        Enum("BAIXA", "MEDIA", "ALTA", "URGENTE", name="prioridade_atendimento_enum"),
        default="MEDIA",
        comment="Nível de prioridade",
    )
    ...
    # Relationships
    aluno = relationship(
        "Aluno", primaryjoin="Aluno.matricula == foreign(Atendimento.aluno_matricula)"
    )
    usuario = relationship("Usuario")
    historico_status = relationship(
        "HistoricoEncaminhamento", back_populates="atendimento", cascade="all, delete-orphan"
    )
...
class HistoricoEncaminhamento(Base):
    ...
    usuario_id = Column(
        ...
        nullable=False,
        index=True,
        comment="Profissional que realizou a mudança",
    )
    status_anterior = Column(
        Enum("SOLICITADO", "EM_ATENDIMENTO", "CONCLUIDO", "CANCELADO"), nullable=True
    )
    status_novo = Column(
        Enum("SOLICITADO", "EM_ATENDIMENTO", "CONCLUIDO", "CANCELADO"), nullable=False
    )
...
class TemplateComunicacao(Base):
    ...
    canal = Column(
        Enum(CanalComunicacao), nullable=False, default=CanalComunicacao.SISTEMA, index=True
    )
...
class ConfiguracaoSistema(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    chave = Column(
        String(50), unique=True, nullable=False, index=True, comment="Identificador único"
    )
...
class Comunicacao(Base):
    ...
    usuario_id = Column(
        ...
        nullable=True,
        index=True,
        comment="Profissional que enviou (NULL = automático)",
    )
    ...
    destinatario_contato = Column(
        String(50), nullable=True, comment="Telefone ou email do destinatário"
    )
    ...
    mensagem = Column(
        Text, nullable=False, comment="Conteúdo da mensagem (com variáveis preenchidas)"
    )
    ...
    data_agendada = Column(
        DateTime(timezone=True), nullable=True, comment="Data/hora para envio agendado"
    )
    data_envio_efetivo = Column(
        DateTime(timezone=True), nullable=True, comment="Quando realmente foi enviado"
    )
...
class Disciplina(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    nome = Column(
        String(100), unique=True, nullable=False, index=True, comment="Nome da disciplina"
    )
    ...
    curso_id = Column(
        ...
        nullable=True,
        comment="ID do curso vinculado (NULL = genérica)",
    )
    criado_at = Column(DateTime(timezone=True), server_default=func.now())

    # Relationships
    curso = relationship("Curso", back_populates="disciplinas")
    faltas_registradas = relationship(
        "RegistroFaltasDiarias", back_populates="disciplina_obj", cascade="all, delete-orphan"
    )
    professores = relationship(
        "DisciplinaProfessor", back_populates="disciplina", cascade="all, delete-orphan"
    )
...
class DisciplinaProfessor(Base):
    ...
    curso_id = Column(
        Integer, ForeignKey("cursos.id"), nullable=True, comment="Cópia para facilitar filtros"
    )
    criado_at = Column(DateTime(timezone=True), server_default=func.now())
...
Usuario.disciplinas = relationship(
    "DisciplinaProfessor", back_populates="usuario", cascade="all, delete-orphan"
)
...
Curso.metas_semestrais = relationship(
    "MetasSemestrais", back_populates="curso", cascade="all, delete-orphan"
)
Aluno.metas = relationship("AlunoMeta", back_populates="aluno", cascade="all, delete-orphan")
Aluno.faltas = relationship(
    "RegistroFaltasDiarias", back_populates="aluno", cascade="all, delete-orphan"
)
Aluno.alertas_faltas = relationship(
    "AlertaFaltasConsecutivas", back_populates="aluno", cascade="all, delete-orphan"
)
Aluno.atendimentos = relationship(
    "Atendimento",
    primaryjoin="Aluno.matricula == foreign(Atendimento.aluno_matricula)",
    back_populates="aluno",
    cascade="all, delete-orphan",
)
Aluno.questionario_psicossocial = relationship(
    "QuestionarioPsicossocial", back_populates="aluno", uselist=False, cascade="all, delete-orphan"
)

...
def verificar_dados_suficientes(db: Session):
    ...
    evasoes = db.query(models.Egresso).filter(models.Egresso.motivo_saida == "ABANDONO").count()
...
def coletar_dados_treinamento(db: Session):
    ...
    matriculas_evadidas = [
        e.aluno_matricula
        for e in db.query(models.Egresso).filter(models.Egresso.motivo_saida == "ABANDONO").all()
    ]
...
def analisar_dados(df):
    ...
    print("\nDistribuição:")
    print(
        f"   0 - Permaneceu: {distribuicao.get(0, 0)} ({distribuicao.get(0, 0)/len(df)*100:.1f}%)"
    )
    ...
    for grupo in [0, 1]:
        ...
        if len(subset) > 0:
            ...
            print(
                f"      Trabalha: {subset['trabalha'].sum()} ({subset['trabalha'].mean()*100:.0f}%)"
            )
...
def salvar_modelo(modelo, acuracia):
    ...
    metadados = {
        ...
        "parametros": {"n_estimators": 200, "max_depth": 15, "class_weight": "balanced"},
    ...
    try:
        ...
        if salvar_modelo(modelo, acuracia):
            ...
            if testar_modelo():
                ...
                print("   2. Novas predições usarão o modelo ML")
                print("   3. Verifique em /predicoes/resumo")
                print("=" * 60)
                return True

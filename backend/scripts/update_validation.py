...
from models import Egresso, NivelRisco, Predicao, PredicaoHistorico
...
with Session(engine) as db:
    ...
    for egresso in egressos:
        ...
        predicao = (
            db.query(Predicao)
            .filter(Predicao.aluno_id == egresso.aluno_matricula)
            .order_by(Predicao.data_predicao.desc())
            .first()
        )
    ...
    resumo = (
        db.query(PredicaoHistorico.tipo_erro, func.count(PredicaoHistorico.id).label("total"))
        .group_by(PredicaoHistorico.tipo_erro)
        .all()
    )
    ...
    vn = (
        db.query(PredicaoHistorico)
        .filter(PredicaoHistorico.tipo_erro == "VERDADEIRO_NEGATIVO")
        .count()
    )
    fp = db.query(PredicaoHistorico).filter(PredicaoHistorico.tipo_erro == "FALSO_POSITIVO").count()
    fn = db.query(PredicaoHistorico).filter(PredicaoHistorico.tipo_erro == "FALSO_NEGATIVO").count()

...
import database
import models
from notificacoes import enviar_alerta_faltas, enviar_alerta_risco
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Envio automático de alertas do SAPEE DEWAS para o Telegram"
    )
    parser.add_argument(
        "--tipo",
        type=str,
        default="todos",
        choices=["todos", "risco", "faltas", "mudanca_risco", "questionario"],
        help="Tipo de alerta a enviar (padrão: todos)",
    )
...
def enviar_mensagem_telegram(mensagem: str, nivel: str = "ALTO"):
    ...
    aluno_fake = {"nome": "Sistema", "matricula": "SYSTEM", "curso": "Geral"}
...
def ja_foi_alertado_recentemente(
    db: Session,
    aluno_matricula: str,
    tipo_alerta: str,
    horas: int = 24,
    ...
    """
    Verifica se um alerta do mesmo tipo já foi enviado para o aluno recentemente.
    Usa a tabela comunicacoes para rastrear envios.
    """
    data_limite = datetime.now() - timedelta(hours=horas)
    existente = (
        db.query(models.Comunicacao)
        .filter(
            models.Comunicacao.aluno_matricula == aluno_matricula,
            models.Comunicacao.tipo_comunicacao == models.TipoComunicacao.RISCO,
            models.Comunicacao.canal == models.CanalComunicacao.SISTEMA,
            models.Comunicacao.status.in_(["ENVIADA", "ENTREGUE", "LIDA"]),
            models.Comunicacao.data_envio >= data_limite,
        )
        .first()
    )
...
def registrar_envio_alerta(
    ...
    """Registra o envio do alerta na tabela comunicacoes"""
    comunicacao = models.Comunicacao(
        aluno_matricula=aluno.matricula,
        usuario_id=usuario_id,
        tipo_comunicacao=models.TipoComunicacao(tipo_comunicacao),
        canal=models.CanalComunicacao.SISTEMA,
        destinatario_tipo=models.DestinatarioTipo.COORDENADOR,
        destinatario_nome="Coordenação",
        destinatario_contato=os.getenv("TELEGRAM_CHAT_ID"),
        assunto=f"Alerta {tipo_comunicacao}",
        mensagem=mensagem,
...
def verificar_risco_alto_e_muito_alto(db: Session):
    """
    Verifica alunos com risco ALTO ou MUITO_ALTO e envia alerta.
    Importante: usado após importação de CSV/base acadêmica.
    Evita spam: só envia se não houver alerta nas últimas 24h.
    """
    print("\n🔍 Verificando alunos com risco ALTO/MUITO_ALTO...")

    # Buscar predições mais recentes de cada aluno
    subquery = (
        db.query(
    ...
    predicoes_alto = (
        ...
        .filter(
            models.Predicao.nivel_risco.in_([
    ...
    alertas_enviados = 0
    niveis_prioridade = {"ALTO": "ALTO", "MUITO_ALTO": "CRITICO"}

    for predicao in predicoes_alto:
        aluno = (
            db.query(models.Aluno)
            .filter(models.Aluno.matricula == predicao.aluno_id)
            .first()
        )
        ...
        # Evitar spam: só enviar se não foi alertado nas últimas 24h
        if ja_foi_alertado_recentemente(
            db, aluno.matricula, "RISCO", horas=24
        ):
            print(f"   ⏭️  {aluno.nome}: já alertado nas últimas 24h")
            continue

        nivel_telegram = niveis_prioridade.get(
            predicao.nivel_risco.value, "ALTO"
        )

        mensagem = f"""
🚨 <b>ALERTA DE RISCO DE EVASÃO - {nivel_telegram}</b>

👤 <b>Aluno:</b> {aluno.nome}
📚 <b>Curso:</b> {aluno.curso.nome if aluno.curso else 'N/A'}
🔴 <b>Nível de Risco:</b> {predicao.nivel_risco.value}
📊 <b>Score:</b> {float(predicao.risco_evasao):.1f}%

<b>Fatores de Risco:</b>
{predicao.fatores_principais or '• Não identificados'}
...
"""
        ...
        try:
            enviar_mensagem_telegram(mensagem, nivel=nivel_telegram)
            registrar_envio_alerta(db, aluno, "RISCO", mensagem)
            print(
                f"   ✅ Alerta enviado: {aluno.nome} ({predicao.nivel_risco.value})"
            )
            alertas_enviados += 1
...
def verificar_faltas_consecutivas(db: Session):
    ...
    faltas_recentes = (
        ...
        .order_by(models.RegistroFaltasDiarias.data.desc())
        .all()
    )
                ...
                if aluno:
                    ...
                    alerta_existente = (
                        ...
                        .filter(
                            ...
                            models.AlertaFaltasConsecutivas.criado_at
                    ...
                    if not alerta_existente:
                        ...
                        novo_alerta = models.AlertaFaltasConsecutivas(
                            ...
                            disciplinas_afetadas=json.dumps(
                                list(set([f.disciplina for f in faltas]))
                            ),
                            status="PENDENTE",
...
"""
                    ...
                    try:
                        enviar_mensagem_telegram(mensagem, nivel="ALTO")
                        print(
                            f"  ✅ Alerta enviado: {aluno.nome} ({risco_anterior} → {risco_atual})"
                        )
                        alertas_enviados += 1
                    except Exception as e:
                        print(f"  ❌ Erro ao enviar alerta: {e}")
...
def verificar_questionario_risco_alto(db: Session):
    ...
    for questionario in questionarios:
        ...
        if not alerta_existente:
            ...
            if aluno:
                fatores = (
                    json.loads(questionario.fatores_criticos)
                    if questionario.fatores_criticos
                    else []
                )
...
"""
                ...
                try:
                    enviar_mensagem_telegram(mensagem, nivel="ALTO")
                    print(
                        f"  ✅ Alerta enviado: {aluno.nome} ({questionario.nivel_risco_psicossocial})"
                    )
                    alertas_enviados += 1
                except Exception as e:
                    print(f"  ❌ Erro ao enviar alerta: {e}")
...
def main():
    ...
    # Parsear argumentos
    args = parse_args()
    print(f"📦 Modo de execução: {args.tipo.upper()}")
    print()
    ...
    try:
        ...
        # Executar verificações conforme o tipo solicitado
        total_alertas = 0

        if args.tipo == "todos":
            total_alertas += verificar_risco_alto_e_muito_alto(db)
        ...
        elif args.tipo == "risco":
            total_alertas += verificar_risco_alto_e_muito_alto(db)
        elif args.tipo == "faltas":
            total_alertas += verificar_faltas_consecutivas(db)
        elif args.tipo == "mudanca_risco":
            total_alertas += verificar_mudanca_risco(db)
        elif args.tipo == "questionario":
            total_alertas += verificar_questionario_risco_alto(db)
